...
function useImcCalcState() {
  ...
  return {
    ...
    derived: { pesoNum, alturaM, imc, classificacao, faixa },
  } as const;
}

const ImcCalcCtx = createContext<ReturnType<typeof useImcCalcState> | null>(
  null
);
  ...
  return (
    <ImcCalcProvider>
      <CalculatorShell
        ...
        subtitle="Informe seu peso e altura (em m ou cm) para calcular o Índice de Massa Corporal de forma rápida."
        heroEmoji="⚖️"
        form={<Form />}
        result={<Result />}
    ...
    >
      ...
      <label className="text-sm">
        ...
        <p className="mt-1 text-xs text-gray-500">
          Dica: pode digitar em <b>metros</b> (1,75) ou <b>centímetros</b>{" "}
          (175).
        </p>
      </label>
...
function Result() {
  ...
  if (!(pesoNum > 0) || !(alturaM > 0)) {
    return (
      <div className="text-sm text-red-600">
        Informe <b>peso</b> e <b>altura</b> válidos.
      </div>
    );
  }

  // checagem simples de faixa "realista" de altura (adultos)
  if (alturaM < 0.5 || alturaM > 2.5) {
    return (
      <div className="text-sm text-red-600">
        Verifique a altura informada. Para adultos, use um valor entre{" "}
        <b>0,50 m</b> e <b>2,50 m</b>.
      </div>
    );
  }
  ...
  let dica = "";
  if (imcVal < 18.5 && Number.isFinite(min)) {
    dica = `Para atingir IMC 18,5, peso alvo ≈ ${round1(min)} kg.`;
  } else if (imcVal > 24.9 && Number.isFinite(max)) {
    dica = `Para ficar em 24,9, peso alvo ≈ ${round1(max)} kg.`;
  }
  ...
  return (
    <div className="space-y-3 text-sm border rounded-xl p-4 bg-gray-50">
      <Info
        title="IMC"
        value={Number.isFinite(imcVal) ? round1(imcVal).toString() : "—"}
      />
      <Info title="Classificação" value={classificacao} />
      {Number.isFinite(min) && Number.isFinite(max) && (
        <Info
          title="Faixa de peso recomendada"
      ...
      <p className="text-xs text-gray-500 mt-3">
        O IMC é um indicador geral e tem limitações (por exemplo: atletas,
        gestantes, idosos). Use como uma ferramenta de triagem e, em caso de
        dúvidas, procure um profissional de saúde.
      </p>
    </div>
  );
}
...
function FaqToggle({ items }: { items: readonly { q: string; a: string }[] }) {
  ...
  const allItems = [
    ...items,
    {
      ...
      a: "Não. A fórmula do IMC é a mesma para ambos os sexos: peso / altura². O que pode variar é a interpretação clínica, já que composição corporal e distribuição de gordura podem ser diferentes. As faixas de referência da OMS, porém, são as mesmas para homens e mulheres adultos.",
    },
  ];
